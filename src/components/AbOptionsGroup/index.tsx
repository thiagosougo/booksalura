...
export interface AbOptionsGroupProps {
    ...
    defaultValue?: AbOptionsGroup | null,
    onChange?: (option: AbOptionsGroup) => void 
}

export const AbOptionsGroup = ({ options, onChange, defaultValue }: AbOptionsGroupProps) => {
    const [selection, setSelection] = useState<AbOptionsGroup | null>(defaultValue ?? null)
    const optionSelected = (option: AbOptionsGroup): void => {
        setSelection(option);
        if (onChange) {
            onChange(option)
        }
